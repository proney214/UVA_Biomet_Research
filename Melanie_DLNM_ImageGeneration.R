...
lagframe=data.frame(Mort=Mort, Trend=Trend, MainSpline=MainSpline,Factor1=Factor1,Factor5=Factor5,datevar=datevar,AccountChange2015=AccountChange2015)
...
modelA1=glm(Mort~cb1.MainSpline+ns(Trend, 16*3)+as.factor(Factor1)+as.factor(Factor5)+as.factor(AccountChange2015),family=quasipoisson(),lagframe)
summary(modelA1)
#plot(modelA1)
...
# Plot of "heat map"
plot(pred1.MainSpline,"contour",xlab="Temp",key.title=title("RR"),plot.title=title("Virginia White Mortality Risk by Temperature and Lag",xlab="Minimum Temperature (\u00B0C)",ylab="Lag (days)"))
