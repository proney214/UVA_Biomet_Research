...
CHONoStressFull <- CHOselect %>%
  filter(HeatLag0Update != 1 & HeatLag1Update != 1 & MortModerate != 1) %>%
...
#Compute Tables
 for (i in (1:1000)) {
   ...
   write.csv(CHONoStressRandom,paste0("~/Desktop/NoStressRandomWx/CHO_v2/CHO",i))
 }
...
CHO_data_all <- list.files(path = "~/Desktop/NoStressRandomWx/CHO_v2/",  # Identify all CSV files
