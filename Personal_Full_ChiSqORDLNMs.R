...
CHOHotDaysModerate[as.numeric(length((CHOHotDaysModerate$HotDaysModerate)+1)), ] = colSums(CHOHotDaysModerate,na.rm=TRUE)
CHOHotDaysModerate[as.numeric(length((CHOHotDaysModerate$HotDaysModerate)+1)), ]

tableprofileHotDaysModerate = data.frame(colSums(CHOHotDaysModerate,na.rm = TRUE))
...
CHOEx <- cbind(minitableColdDaysModerate,minitableHotDaysModerate,minitableNoStress)
colnames(CHOEx) <- c("x","ColdDaysModerate","x","HotDaysModerate","x","NoStress")
CHOExshort <- data.frame(CHOEx$ColdDaysModerate,CHOEx$HotDaysModerate,CHOEx$NoStress)
View(CHOExshort)
...
tableprofileColdDaysModerate = data.frame(colSums(CHOColdDaysModerate,na.rm = TRUE))
...
modelMDC.04=glm(MDC.04~cb1.ATC1pm+ns(PM2.5REVISED, 3)+ns(Trend, 16*4),family=poisson(),lagframe)
summary(modelMDC.04)
#plot(modelMDC.04)
...
plot(pred1.ATC1pm,ptype="slices",var=c(-5),col='black',xlab='lag (days)',ylab="Relative Risk",main="CHO MDC.04: RR vs. Lag (days) at 0°C (ATC1pm)",ylim=c(.8,1.1))
...
###DLNM for MDC.05
modelMDC.05=glm(MDC.05~cb1.ATC1pm+ns(PM2.5REVISED, 3)+ns(Trend, 16*4),family=poisson(),lagframe)
summary(modelMDC.05)
#plot(modelMDC.05)
...
###DLNM for MDC.01
modelMDC.01=glm(MDC.01~cb1.ATC1pm+ns(PM2.5REVISED, 3)+ns(Trend, 16*4),family=poisson(),lagframe)
summary(modelMDC.01)
#plot(modelMDC.01)
...
###DLNM for Mort
modelMort=glm(Mort~cb1.ATC1pm+ns(PM2.5REVISED, 3)+ns(Trend, 16*4),family=poisson(),lagframe)
summary(modelMort)
#plot(modelMort)
