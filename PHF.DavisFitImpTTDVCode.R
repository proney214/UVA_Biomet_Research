...
library(dplyr)
#
# Create Null Data Frame FOR SHD
#
df=data.frame(col1=numeric(),col2=numeric(),col3=numeric(),col4=numeric())

Station.test=read_excel("PHF.working.xlsx",sheet = "Sheet1",na="NA")
...
write.csv(df,"~/Desktop/EVSC_Research/PHF.test.DavisFill.output3.csv",row.names=FALSE)
#
...
library(imputeTS)
raw <- read.csv("PHF.test.DavisFill.output3.csv", sep = ",")
Final <- na_interpolation(raw)
write.csv(Final, "~/Desktop/EVSC_Research/PHF.DavisFill.FullInt3.csv")
