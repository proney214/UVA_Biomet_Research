...
HeatWavesModerate <- data.frame(CHO$HeatWavesModerate)
mort <- data.frame(CHO$Mort)

HW <- HeatWavesModerate
...
colnames(desiredtable) <- c("HeatWaveCount", "HeatWaveDuration","RowNumber")
