...
deathproj <- rep(deathdoy,length=nrow(obs))
...
obs$doy <- seq(1,length(obs$Station),1)
#names(obs)[names(obs) == ''] <- 'date'
...
plot(all ~ doy,obs,pch=19,col=grey(0.8),cex=0.5,ylab="Daily deaths",xlab="Day of the year",
     main="")
lines(1:length(obs$doy),tapply(obs$all,obs$doy,mean),lwd=1.5,col=4)
#dev.off()
...
# LOAD THE PACKAGES
library(dlnm) ; library(splines) ; library(MASS) ; library(tsModel) ; library(readxl)
...
obs <- read_excel("~/Desktop/PHF.Final.Weather.xlsx")
obs$Date <- as.Date(obs$Date)#, format="%d/%m/%Y")
names(obs)[names(obs) == 'MaxT(C)'] <- 'tmean'
...
names(obs)[names(obs) == 'Mort'] <- 'all'
names(obs)[names(obs) == 'MaxT(C)'] <- 'tmean'
names(obs)[names(obs) == 'Date'] <- 'date'
